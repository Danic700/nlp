...
def clean_text(text, stop_words, chars_to_remove = punctuation_characters, join_spaces = False, valid_letters = None):
	"""
	Tokenize text, remove stop words, handle dot character(if necessary)
	Returns tuple of (tokens, word types)
	"""	
	
	clean_text = _remove_unwanted_characters(text, chars_to_remove).lower().replace('--', '')
	...
	if join_spaces:
		...
		return (filtered_words, filtered_words_set, clean_text)
