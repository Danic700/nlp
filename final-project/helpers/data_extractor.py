...
class DataExtractor:
	...
	def extract_data(self, data, text_columns):
		...
		for line in data.split('\n'):
			...
			extracted_data.append([
			    self._remove_sentence_unwanted_words(subject),
			    self._remove_sentence_unwanted_words(content), category
			])
	...
	def _remove_text_unwanted_words(self, text):
		import re
		unwanted_words = ['&#xd;', '&lt;', 'br', '&gt;']
		p = re.compile('|'.join(map(re.escape,
		                            unwanted_words)))  # escape to handle metachars
		return p.sub('', text)
	...
	def _remove_sentence_unwanted_words(self, sentence):
		...
		wanted_letters = [
		    letter for letter in tokens if not letter in letters_to_remove
		]
		wanted_words = " ".join(wanted_letters)
		stop_words = set(stopwords.words("english"))
		unwanted_words = set(['get']).union(stop_words)
		wanted_words_list = " ".join([
		    word for word in wanted_words.split(" ") if not word in unwanted_words
		])
		filtered_words = [
		    word for word in wanted_words_list.split(" ")
		    if len(word) > 2 and not word.replace('.', '', 1).isdigit()
		]
